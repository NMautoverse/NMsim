...
##'     demonstration.
##'
##' Only `EVID%in%c(0,2)` rows in `data` are used for calculations.
##' 
...
### modelby are NMsim model columns that will be used if found
    ...
    simres <- as.data.table(data)[EVID%in%c(0,2)]
    if(nrow(data)==0) stop("Data set has no `EVID%in%c(0,2)` rows.")
...
### making reference value a column rather than rows. 
    ...
    sum.res.model <- mergeCheck(
sum.res.model[type=="value"]
,
                                dt.ref
                               ,
