...
NMwriteInitsOne <- function(lines,inits.w,inits.orig,pars.l){
  ...
  paste.ll.init.ul <- function(lower,init,upper,FIX,BLOCK){
    ...
    if(!is.na(init)){
      ...
      ## dt[init!="SAME"&is.na(BLOCK),res:=paste0("(",paste(setdiff(c(lower,init,upper),NA),collapse=","),")",FIX),by=row]
      ## dt[init!="SAME"&is.na(BLOCK)&is.na(lower)&is.na(upper),res:=paste0(init,FIX),by=row]
      
      ## if(is.na(upper) ) browser()
 ...
 inits.w[,elemnum:=min(elemnum_lower,elemnum_init,elemnum_upper,elemnum_BLOCK,na.rm=TRUE),by=row]
  ...
  ## Insert $section in front of all parameter lines
  ...
  ## lines.all.2 <- elems.all[,.(newtext=paste(string.elem,sep=" ")),keyby=.(par.type,linenum)]
