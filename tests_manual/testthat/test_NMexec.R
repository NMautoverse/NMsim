...
test_that("fun.post",{
    ...
    fileRef <- "testReference/NMexec_06.rds"

    file.mod <- "testData/nonmem/xgxr022quick.mod"

    fun.post <- function(file.mod){
        sprintf("echo \"This was generated by NMexec after execution of '%s'\" > fun_post_out.txt",file.mod)
    }
    ## fun.post(file.mod)
    file.out <- "testData/nonmem/fun_post_out.txt"
    if(file.exists(file.out)) unlink(file.out)
    
    NMexec(file.mod , path.nonmem=path.nonmem,sge=F,fun.post=fun.post)

    res <- readLines(file.out)
    expect_equal_to_reference(res,fileRef)
