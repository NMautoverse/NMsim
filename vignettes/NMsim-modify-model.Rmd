...
NMdataConf(path.nonmem = NMsim:::prioritizePaths(path.candidates)) ## path to whichever NONMEM executable that is available

NMdataConf(dir.sims="simtmp-modify"
...
values have been updated in the simulation control stream (to the right) based on the
...
properties), `modify` edits the control stream text directly. This is
a powerful alternative because it can allow for modifications
independent of parameter numbering, and the model can be modified very
freely.
...
```{r diff-ka,echo=FALSE}
my.file.diff(modTab(simres)$path.sim,
             modTab(simres.ka4)$path.sim,sections="PK",
             x_arg="basic-sim",y_arg="sim-ka4")
```

The `modify` argument takes a list, where each element is named as the control stream section to be modified, in this case `$PK`. It can be 
- A character vector: the entire secion is overwritten
- A function: The function is applied to the section text
...
`NMsim::add()` may be a little easier to write and read. It can also
prepend text, see the `pos` argument. There is also a function called
`NMsim::overwrite()` which can replace text strings. Once you try this
one or two times, you will hopefully appreciate how easily you can
tweak control streams to do what you need, still benifiting from
`NMsim()`'s handling of data, automated control stream generation etc.
...
(b) the specific line that defines `TVKA` is modified to include the effect of the scaling factor `KASCALE`. This is done for illustration of `overwrite`, see the `inits` argument for how to edit parameter values directly.
...
<details>
<summary>`modify` example: Drug-Drug Interaction (DDI)</summary>
...
<details>
<summary>`modify` example: Conditional Dose Delay</summary>
...
<details>
<summary>`modify` example: Calculation of AUC</summary>
...
[VPC Simulations](NMsim-VPC.html#change-the-acceptignore-statements)
