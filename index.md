# NMsim - Simulate Nonmem Models from R
...
![](reference/figures/README-simple-sim-1.png)
...
![NMsim can simulate NONMEM models right away - but only ifh NONMEM is
avaiable.](reference/figures/intro-tab.png)
...
- Provides a simple interface
  ...
  to read previously performed simulation results.
...
However, running Nonmem using the
[`NMexec()`](https://nmautoverse.github.io/NMsim/reference/NMexec.md)
function provided by `NMsim` has one important advantage in that it
saves the input data together with the Nonmem control streams. This
ensures that output data can be merged with input data as it went into
the model, even if the input data file should be modified or lost.
